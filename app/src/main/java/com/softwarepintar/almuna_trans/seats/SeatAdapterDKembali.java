...
class SeatAdapterDKembali extends RecyclerView.Adapter<SeatAdapterDKembali.ViewHolder> {
    private Context context;
    private List<ResultDItem>resultDItemList;

    public SeatAdapterDKembali(Context context, List<ResultDItem> resultDItemList) {
    ...
    @NotNull
    @Override
    public SeatAdapterDKembali.ViewHolder onCreateViewHolder(@NonNull @NotNull ViewGroup parent, int viewType) {
        View view= LayoutInflater.from(parent.getContext()).inflate(R.layout.list_kursi_d,parent,false);
        return new SeatAdapterDKembali.ViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull @NotNull SeatAdapterDKembali.ViewHolder holder, int position) {
...
//                    Toast.makeText(context,"show "+String.valueOf(Utils.seat_selected.size()), Toast.LENGTH_LONG).show();
                    ...
                    if (Utils.seat_selected2.size() <= Integer.parseInt(PilihKursiActivity.jumlah)){
                        ...
                        Utils.seat_selected2.add(resultDItem.getSeatNama());
